...
function detail(id){const p=displayed.find(p=>p.id===id);$('detail-content').innerHTML=`<h2>${esc(p.name)}</h2><p>Consensus ${fmt(p.mean)} · Raw average ${fmt(p.raw_mean)} · Median ${fmt(p.median)}</p><table><thead><tr><th>SOURCE</th><th>RANK</th><th>VOTE</th></tr></thead><tbody>${p.ranks.map(r=>{const s=data.sources.find(s=>s.id===r.source);return `<tr><td><a href="${esc(s.url)}" target="_blank" rel="noopener">${esc(s.name)}</a><small>${esc(s.kind||'Individual analyst')} · ${String(r.published).startsWith('Retrieved ')?esc(r.published):'Updated '+esc(r.published)+' (publisher time)'}</small></td><td>${r.rank}</td><td class="${p.excluded.includes(r.source)?'excluded':''}">${p.excluded.includes(r.source)?'Excluded outlier':'Included'}</td></tr>`;}).join('')}</tbody></table><p>Excluded votes remain visible. Filtering is symmetric and does not target any particular analyst.</p>`;$('detail').showModal();}
...
async function start(){
  ...
  $('source-summary').textContent=`${data.sources.length} configured sources · ${data.sources.filter(s=>data.health.some(h=>h.source===s.id&&h.status==='available')).length} currently contributing. Individual analysts and publisher models may share information; they are not statistically independent. Free availability is checked each update. New sources are not covered by the seven-analyst historical audit.`;
